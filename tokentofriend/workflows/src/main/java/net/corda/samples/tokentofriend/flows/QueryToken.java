...
public class QueryToken extends FlowLogic<String>{
    ...
    public String call() throws FlowException {
        ...
        try {
            QueryCriteria.LinearStateQueryCriteria inputCriteria = new QueryCriteria.LinearStateQueryCriteria().withUuid(Arrays.asList(UUID.fromString(uuid)))
                    .withStatus(Vault.StateStatus.UNCONSUMED);
            receivedToken = getServiceHub().getVaultService().queryBy(NonFungibleToken.class).getStates().get(0).getState().getData();
        ...
        try {
            QueryCriteria.LinearStateQueryCriteria inputCriteria = new QueryCriteria.LinearStateQueryCriteria().withUuid(Arrays.asList(UUID.fromString(tokenTypeStateId)))
                    .withStatus(Vault.StateStatus.UNCONSUMED);
            underlineState = getServiceHub().getVaultService().queryBy(CustomTokenState.class).getStates().get(0).getState().getData();
        }catch(NoSuchElementException e){
